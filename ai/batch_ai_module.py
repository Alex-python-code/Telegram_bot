...
class Ai:
    ...
    def status_checker(self, batch_id):
        while True:
            ...
            if status.status in ['validating', 'queued', 'in_progress']:
                ...
                print(f'Задача batch id {status.id} завершена с ошибкой: {datetime.now()}, статус: {status.status}')
    ...
    def responses_to_dict(self, response_of_jsonl):
        list_of_responses = [json.loads(line) for line in response_of_jsonl.splitlines() if line.strip()]
        print(f'Файл успешно был конвервертирован в список словарей')
