...
with Session() as session:
    ...
    """Ежедневное добавление количества юзеров"""
    try:
        session.add(Users_statistics(day=date_of_day, all_users=today_count_users))
    except Exception as e:
        logger.error(
            f"Не удалось добавить количествой пользователей в all_users по причине: {e}"
        )
    ...
    try:
        ...
        logger.error(
            f"Не удалось получить количество активных пользователей за {datetime.now().replace(microsecond=0)} по причине: {e}"
        )
