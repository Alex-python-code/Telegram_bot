...
class TimeUtils:
    ...
    def check_time(news_block, html_time_element, html_time_class):
        ...
        news_time = TimeUtils.get_news_time(
            news_block, html_time_element, html_time_class
        )
...
class Parser:
    ...
    def __init__(
        self,
        url,
        parent_html_news_element,
        parent_html_news_class,
        html_news_element,
        html_news_class,
        sublink_element,
        sublink_class,
        next_page_link_element,
        next_page_link_class,
        class_of_news_blocks,
        main_site,
        time_html_class,
        time_html_element,
        is_mass_media,
    ...
    def _html_subpages_parser(self):
        ...
        for i in self.temp:
            # print(i.text)
            if not TimeUtils.check_time(
                i, self.time_html_element, self.time_html_class
            )[0]:
                if stop_parsing:
                    self._run = False
                    return all_new_body
                continue
            self.now_time_interval = TimeUtils.check_time(
                i, self.time_html_element, self.time_html_class
            )[1]
            ...
            if _subsoup:
                try:
                    new_body = _subsoup.find(
                        "div", class_="topic-page__container"
                    ).find_all("p")
                    label = _subsoup.find(
                        "span", class_="topic-body__title"
                    )  #!!!Обратить внимание!!!
            ...
            for news_string in new_body:
                if not any(
                    word in str(news_string.text).lower()
                    for word in self.exclude_word_in_news
                ):
    ...
    def _get_next_page_link(self):
        ...
        if (
            self.next_page_link_element
            and self.next_page_link_class
            and self._main_site
        ):
            next_page_link = self._soup.find_all(
                self.next_page_link_element, class_=self.next_page_link_class
            )
            if len(next_page_link) > 1:
                self.url = self._get_full_link(
                    self._main_site, next_page_link[1]["href"]
                )
            elif self._cnt < 2 and self._is_first_page:
                self._is_first_page = False
                self.url = self._get_full_link(
                    self._main_site, next_page_link[0]["href"]
                )
            else:
                print(
                    f"Ссылка для следующей страницы ресурса {self.url} не была найдена"
                )
                self._run = False
    ...
    def main_page_parser(self, attempts):
        ...
        while self._run:
            try:
                ...
                print(
                    f"Ошибка при получении html кода главной страницы {self.url}: {e}"
                )
                if attempts < 3:
            ...
            if self._soup:
                self.temp = self._soup.find(
                    self.parent_html_news_element, class_=self.parent_html_news_class
                ).find_all(self.html_news_element, class_=self.html_news_class)
            else:
