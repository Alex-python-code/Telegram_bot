        ...
        while True:
            ...
            for field in required_fields:
                if not getattr(source_info, field, None):
                    print(
                        f"Отсутствуют данные о ресурсе {source_info.source_url} в блоке {field}"
                    )

            full_link = self.make_full_link(
                source_info.source_url, source_info.date_format
            )
            parser = Parser(
                url=full_link,
            ...
            while True:
                ...
                if isinstance(parser_response, int):
                    continue
                elif not parser_response:
                    print("Парсер завершил работу с ошибкой")
                    return False
                elif parser_response:
